...
data %<>% mutate(PTSD_avoid_T1 = select(., T1PTSD5, T1PTSD6) %>% rowMeans,
                 PTSD_avoid_T2 = select(., T2PTSD5, T2PTSD6) %>% rowMeans,
                 PTSD_avoid_T3 = select(., T3PTSD5, T3PTSD6) %>% rowMeans)
...
to_melt_suffix <- c('PTSD_T', 'PTSD_avoid_T', 'Cope_T', 'Fatalism_T', 'Depmean_T', 'SocCohmean_T', 'Anxmean_T', 'DP_cleaned_T', 'DP_cleaned_excludedItems_T', 'ChronStress_T')
...
data %<>% mutate(Depmean_delta_T1T2 = Depmean_T2 - Depmean_T1, 
                 ...
                 PTSD_avoid_delta_T1T2 = PTSD_avoid_T2 - PTSD_avoid_T1,
...
```{r, cache = TRUE}
PTSD_avoid_T_model <- lmer(PTSD_avoid_T ~ interventiongroup * timePoint_factor + (1|Locationcode/T1ParticipantID), data = filtered)
summary(PTSD_avoid_T_model)
Anova(PTSD_avoid_T_model, type = "III")
summary(rbind(pairs(lsmeans::lsmeans(PTSD_avoid_T_model, ~ interventiongroup * timePoint_factor), by = "timePoint_factor"))[2]) 
```
